...
const FALLBACK_PATHEXT: &str = ".COM;.EXE;.BAT;.CMD;.VBS;.VBE;.JS;.JSE;.WSF;.WSH;.MSC;.CPL;.PY;.PYW";
...
pub fn execute_command(command: &str, timeout_secs: u64, shell_override: Option<&str>) -> (String, i32) {
    let shell = shell_override.map(str::to_string).unwrap_or_else(pick_shell);
    ...
    if !pending.wait_until(&mut child, &stdout_rx, &stderr_rx, Instant::now() + Duration::from_secs(timeout_secs)) {
        // Stage 1: ask the process group to exit gracefully.
        let pid = child.id();
        unsafe {
            let _ = GenerateConsoleCtrlEvent(CTRL_BREAK_EVENT, pid);
        }
        if !pending.wait_until(&mut child, &stdout_rx, &stderr_rx, Instant::now() + GRACE_PERIOD) {
            ...
            let _ = pending.wait_until(&mut child, &stdout_rx, &stderr_rx, Instant::now() + GRACE_PERIOD);
        }
...
fn pick_shell() -> String {
    if which("pwsh").is_some() { "pwsh".to_string() } else { "powershell".to_string() }
}
...
fn which(name: &str) -> Option<std::path::PathBuf> {
    ...
    let exts: Vec<String> = pathext.split(';').filter(|e| !e.is_empty()).map(str::to_lowercase).collect();
...
fn prepare_env() -> HashMap<String, String> {
    ...
    if !machine_path.is_empty() || !user_path.is_empty() {
        ...
        env.insert("PATH".to_string(), dedup_path(&[&current_path, &machine_path, &user_path]));
    }

    let effective_pathext = if !user_pathext.is_empty() { user_pathext } else { machine_pathext };
    let current_pathext_has_exe = env.get("PATHEXT").is_some_and(|v| v.to_uppercase().contains(".EXE"));
    ...
    env.entry("USERPROFILE".to_string()).or_insert_with(|| user_profile.clone());
...
fn read_reg_env(root: &windows_registry::Key, subkey: &str) -> (HashMap<String, String>, String, String) {
...
fn dedup_path(segments: &[&str]) -> String {
    let joined = segments.iter().filter(|s| !s.is_empty()).copied().collect::<Vec<_>>().join(";");
...
mod tests {
    ...
    fn split_drive_extracts_drive_letter() {
        assert_eq!(split_drive(r"C:\Users\foo"), ("C:".to_string(), r"\Users\foo".to_string()));
        assert_eq!(split_drive(r"\\server\share"), (String::new(), r"\\server\share".to_string()));
    }

    #[test]
    fn shell_basename_lower_strips_extension_and_path() {
        assert_eq!(shell_basename_lower(r"C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe"), "powershell");
