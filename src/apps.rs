...
pub fn get_apps_from_start_menu() -> HashMap<String, String> {
    let (output, status) = powershell::execute_command("Get-StartApps | ConvertTo-Csv -NoTypeInformation", 10, None);
...
fn parse_start_apps_csv(csv_text: &str) -> HashMap<String, String> {
    ...
    for record in reader.records().flatten() {
        ...
        if !name.is_empty() && !appid.is_empty() {
            apps.entry(name.to_lowercase()).or_insert_with(|| appid.to_string());
        }
    }
    apps
}
    ...
    for base in bases {
        ...
        for lnk in find_lnk_files(base_path) {
            if let Some(stem) = lnk.file_stem().and_then(|s| s.to_str()) {
                apps.entry(stem.to_lowercase()).or_insert_with(|| lnk.to_string_lossy().into_owned());
            }
        }
    }
    apps
...
pub fn launch_app(name: &str) -> (String, i32, Option<u32>) {
    ...
    let Some((matched_key, _)) = crate::fuzzy::extract_one(name, keys, 70.0) else {
        return (format!("{} not found in start menu.", title_case(name)), 1, None);
    };
    ...
    if Path::new(appid).exists() || appid.contains('\\') {
        ...
        let command = format!("Start-Process {} -PassThru | Select-Object -ExpandProperty Id", ps_quote(&exe_path));
        let (response, status) = powershell::execute_command(&command, 10, None);
        let pid = if status == 0 { response.trim().parse::<u32>().ok() } else { None };
        (response, status, pid)
    } else {
        if !check_app_exists(appid) {
            return (format!("Invalid app identifier: {appid}"), 1, None);
        }
        let command = format!("Start-Process {}", ps_quote(&format!("shell:AppsFolder\\{appid}")));
        let (response, status) = powershell::execute_command(&command, 10, None);
