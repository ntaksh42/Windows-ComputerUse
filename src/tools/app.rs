...
pub fn app(params: AppParams) -> Result<String, String> {
    let AppParams { mode, name, window_loc, window_size, executable, args, cwd } = params;
...
fn to_pair(value: Option<ListOrString<i32>>, field: &str) -> Result<Option<(i32, i32)>, String> {
    ...
    match list.as_slice() {
        [x, y] => Ok(Some((*x, *y))),
        _ => Err(format!("{field} must have exactly 2 elements [x, y], got {}", list.len())),
    }
}
...
fn launch(name: Option<&str>) -> String {
    ...
    if window::wait_for_window(pid, name, Duration::from_secs(10)) {
        ...
        format!("Launching {} sent, but window not detected yet.", title_case(name))
    }
}
...
fn switch(name: Option<&str>) -> String {
    ...
    if was_minimized {
        format!("Restored {} from minimized and switched to it.", title_case(&target.title))
    } else {
        format!("Switched to {} window.", title_case(&target.title))
    }
}

fn launch_executable(executable: &str, args: Vec<String>, cwd: Option<&str>) -> Result<String, String> {
    ...
    let child = command.spawn().map_err(|e| format!("Failed to launch executable: {e}"))?;
...
fn resolve_executable(executable: &str) -> Result<String, String> {
    ...
    let absolute = std::path::absolute(&expanded).unwrap_or_else(|_| Path::new(&expanded).to_path_buf());
    if !absolute.is_file() {
...
fn resolve_cwd(cwd: &str) -> Result<String, String> {
    ...
    let absolute = std::path::absolute(&expanded).unwrap_or_else(|_| Path::new(&expanded).to_path_buf());
    if !absolute.is_dir() {
        return Err(format!("Working directory does not exist: {}", absolute.display()));
    }
    Ok(absolute.display().to_string())
}
